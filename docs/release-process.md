...
- The release benchmark bundle is generated by the release workflow; review it before publishing a performance claim.
...
Every GitHub Release workflow run creates and attaches an
`oxdoc-vX.Y.Z-benchmarks.tar.gz` asset. It contains:

- `metadata.json`: release tag and commit, commands, runner/platform details,
  and Rust, Cargo, and Python versions;
- `criterion/`: raw Criterion estimates and sample data for the in-process
  parser throughput suite;
- `peak-memory.md`: three-sample median peak RSS for synthetic release-CLI
  workloads; and
- `README.md`: scope and comparison limits.

The workflow builds the release CLI, runs `cargo bench --locked -p oxdoc-core
--bench throughput`, then runs the peak-memory script against that binary. It
validates that the bundle has metadata, a memory report, and Criterion estimate
files before attaching it. The tagged source and recorded commands make the
workflow repeatable; measurement values are not expected to be bit-for-bit
reproducible across machines.

When reviewing or publishing a claim, download the asset for that release and
cite its tag, commit, command, and environment from `metadata.json`. Compare
results only with releases produced under materially equivalent runner/OS,
architecture, tooling, command, and fixture settings. GitHub-hosted runners are
useful for versioned release evidence, not stable dedicated benchmark hardware.

The optional competitor workbench is intentionally not run or published by the
release workflow. Its installed tools, versions, startup costs, and output
semantics vary. If publishing one of those comparisons, run it locally with at
least three iterations and publish the generated report, exact command, tool
versions, and environment alongside the claim.
...
- `oxdoc-vX.Y.Z-benchmarks.tar.gz` (versioned benchmark evidence)

Each binary archive contains the `oxdoc` binary, `README.md`, `LICENSE`, and
`install.sh`. The Windows binary archive contains `oxdoc.exe`.
