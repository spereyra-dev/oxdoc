...
use crate::xlsx_schema::{self, XlsxLogicalType, XlsxSchemaReport};
...
impl Error {
    /// Stable machine-readable category for programmatic error handling.
    pub const fn code(&self) -> &'static str {
        match self {
            Self::EmptySchema => "empty_schema",
            Self::EmptyColumnName { .. } => "empty_column_name",
            Self::DuplicateColumnIndex { .. } => "duplicate_column_index",
            Self::NonNullableNullColumn { .. } => "non_nullable_null_column",
            Self::InvalidBatchRows => "invalid_batch_rows",
...
/// Stable writer configuration for bounded Parquet output.
#[derive(Debug, Clone)]
pub struct ParquetWriteOptions {
    /// Maximum rows per Arrow batch and explicitly flushed Parquet row group.
    pub batch_rows: usize,
    /// Properties forwarded to the Parquet writer.
...
/// Write Parquet with explicit batch/row-group and writer properties.
pub fn write_xlsx_parquet_from_reader_with_options<R, W>(
    reader: R,
    options: XlsxSheetOptions<'_>,
    schema: &TabularSchema,
    ...
    W: Write + Send,
{
    write_xlsx_parquet_from_reader_with_policy(
        ...
        options,
        schema,
        write_options.batch_rows,
        Utf8CoercionPolicy::Strict,
        Some(write_options.writer_properties),
        writer,
    )
}

/// Atomically write a Parquet file beside its final destination.
///
/// The temporary file is removed on failure and renamed only after the writer
/// closes successfully. Existing destinations are replaced atomically where
/// the platform supports same-filesystem replacement.
pub fn write_xlsx_parquet_to_path_atomic(
    input: impl AsRef<Path>,
    sheet_options: XlsxSheetOptions<'_>,
    schema: &TabularSchema,
    ...
    for _ in 0..100 {
            ...
            .open(&candidate)
        {
            ...
            Err(error) => return Err(OxdocError::from(error).into()),
        }
    }
    let (temporary, writer) = temporary.ok_or_else(|| {
        OxdocError::from(std::io::Error::new(
            std::io::ErrorKind::AlreadyExists,
            "could not reserve a unique adjacent temporary output file",
        ))
    })?;
    let result = (|| {
        let input = std::fs::File::open(input).map_err(OxdocError::from)?;
        let extraction = write_xlsx_parquet_from_reader_with_options(
            input,
            sheet_options,
            schema,
            write_options,
        ...
        std::fs::rename(&temporary, output).map_err(OxdocError::from)?;
        Ok(extraction)
    })();
    if result.is_err() {
...
where
    ...
    W: Write + Send,
{
    let mut parquet = ArrowWriter::try_new(writer, schema.arrow_schema(), writer_properties)?;
...
mod tests {
    ...
    #[test]
    fn applies_writer_options_and_reports_stable_json() {
        let schema = test_schema();
        let mut output = Vec::new();
        let extraction = write_xlsx_parquet_from_reader_with_options(
            workbook(&[("1", "alpha"), ("2", "beta"), ("3", "gamma")]),
            ...
            ParquetWriteOptions {
                ...
                writer_properties: WriterProperties::builder()
                    .set_created_by("oxdoc-tabular-test".to_owned())
                    .build(),
            },
            &mut output,
        )
        ...
        assert_eq!(
            serde_json::to_value(extraction.value).unwrap(),
            serde_json::json!({"rows": 3, "batches": 2, "row_groups": 2})
        );
        let reader = SerializedFileReader::new(bytes::Bytes::from(output)).unwrap();
        assert_eq!(
            reader.metadata().file_metadata().created_by(),
            Some("oxdoc-tabular-test")
        );
    }

    #[test]
    fn atomic_path_writer_replaces_only_after_success() {
        let schema = test_schema();
        let input = temp_path("tabular-atomic-input.xlsx");
        let output = temp_path("tabular-atomic-output.parquet");
        fs::write(
            &input,
            workbook(&[("1", "alpha"), ("2", "beta")]).into_inner(),
        )
        ...
        let extraction = write_xlsx_parquet_to_path_atomic(
            &input,
            XlsxSheetOptions::default(),
            &schema,
            &output,
        ...
        .unwrap();
        assert_eq!(extraction.value.row_groups, 2);
        let reader = SerializedFileReader::new(fs::File::open(&output).unwrap()).unwrap();
        assert_eq!(reader.metadata().file_metadata().num_rows(), 2);

        fs::write(&input, workbook(&[("invalid", "alpha")]).into_inner()).unwrap();
        fs::write(&output, b"keep me").unwrap();
        let error = write_xlsx_parquet_to_path_atomic(
            &input,
            XlsxSheetOptions::default(),
            &schema,
            &output,
    ...
    #[test]
    fn converts_formula_cells_from_cached_values() {
        let schema = TabularSchema::new(vec![Column::new(
            "calculated",
            0,
            TabularType::Int64,
            false,
